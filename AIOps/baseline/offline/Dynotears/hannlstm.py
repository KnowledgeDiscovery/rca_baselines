...
class hannLSTM(nn.Module):
    def __init__(self, num_series, hidden):
        '''
        hannLSTM model with one LSTM per time series.

        Args:
          num_series: dimensionality of multivariate time series.
          hidden: number of units in LSTM cell.
        '''
        super(hannLSTM, self).__init__()
        self.p = num_series
        self.hidden = hidden
    ...
    def GC(self, threshold=True):
        '''
        Extract learned causality.

        Args:
          threshold: return norm of weights, or whether norm is nonzero.

        Returns:
          CE: (p x p) matrix. Entry (i, j) indicates whether variable j is
             causal of variable i.
        '''
...
def train_model_pgdls(hannLSTM, X, context, lam, lam_ridge, lr, max_iter,
    ...
    '''
    Train hannLSTM model with PGDLS.

    Args:
      hannLSTM: hannLSTM model.
      ...
      lr: learning rate.
      max_iter: max number of PGDLS iterations.
    ...
    '''
    p = hannLSTM.p
    hannLSTM_copy = deepcopy(hannLSTM)
    ...
    for it in range(max_iter):
        ...
        # Perform PGDLS step for each network.
        for i in range(p):
            ...
            lr_it = lr_list[i]
            net = hannLSTM.networks[i]
            net_copy = hannLSTM_copy.networks[i]

            while not step:
                # Perform tentative PGDLA step.
            ...
            if step:
                ...
                hannLSTM.networks[i], hannLSTM_copy.networks[i] = net_copy, net
        ...
        if (it + 1) % check_every == 0:
            ...
            if verbose > 0:
              ...
              #        % (100 * torch.mean(hannLSTM.GC().float())))
...
def train_model_adam(hannLSTM, X, context, lr, max_iter, lam=0, lam_ridge=0,
                     lookback=5, check_every=50, verbose=1):
    '''Train model with Adam.'''
    p = X.shape[-1]
    loss_fn = nn.MSELoss(reduction='mean')
    optimizer = torch.optim.Adam(hannLSTM.parameters(), lr=lr)
    train_loss_list = []
    ...
    for it in range(max_iter):
        # Calculate loss.
        pred = [hannLSTM.networks[i](X)[0] for i in range(p)]
        loss = sum([loss_fn(pred[i][:, :, 0], Y[:, :, i]) for i in range(p)])

        # Add penalty term.
        if lam > 0:
            loss = loss + sum([regularize(net, lam) for net in hannLSTM.networks])

        if lam_ridge > 0:
            loss = loss + sum([ridge_regularize(net, lam_ridge)
                               for net in hannLSTM.networks])
    ...
    # Restore best model.
    restore_parameters(hannLSTM, best_model)

    return train_loss_list

def train_model_pgd(clstm, X, context, lr, max_iter, lam=0, lam_ridge=0,
                     lookback=5, check_every=50, verbose=1):
    '''Train model with PGD.'''
    p = X.shape[-1]
    loss_fn = nn.MSELoss(reduction='mean')
    train_loss_list = []
...
def train_unregularized(hannLSTM, X, context, lr, max_iter, lookback=5,
