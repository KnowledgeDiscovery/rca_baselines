...
if __name__ == '__main__':
    ...
    if columns_common:
        columns_common = list(set(log_data[log][log_label]['Node_Name']).intersection(columns_common))
    else:
        columns_common = list(log_data[log][log_label]['Node_Name'])
    columns_common.sort()
    ...
    log_length = len(str(log_data[log][log_label]['time'][1]))
    scale = 1
    ...
    kpi_data = np.zeros(log_data[log][log_label]['time'].shape)
    KPI_timestamp = 0
    j = 0
    ...
    for i, log_timestamp in enumerate(log_data[log][log_label]['time']):
        ...
        if KPI_timestamp == log_timestamp:
            ...
            continue
    kpi_data = kpi_data[:log_data[log][log_label]['Sequence'].shape[1]]
    ...
    for metric, weight in POD_METRIC_FILE.items():
        ...
        print('Shape of X', metric_data[metric][label]['Sequence'].shape)
        metric_data[metric][label]['Sequence'] = metric_data[metric][label]['Sequence'].sum(axis=2).transpose()
        print('Shape of X after transpose', metric_data[metric][label]['Sequence'].shape)
    ...
    for metric in metric_names:
        ...
        # cg = generate_causal_graph(X, method='golem')
        #LSTM based method
        cg = generate_causal_graph(X, method='lstm', args={'hidden': 50, 'context': 50, 'lam': 10., 'lam_ridge': 1e-2, 'lr': 1e-3, 'max_iter': 3000, 'check_every': 100, 'device': torch.device('cuda')})
    ...
    # pod_results_combine = pod_results_combine
    ...
    output_file = os.path.join(pathset, 'Pod_level_combine_ranking_log_{}.csv'.format(dataset))
