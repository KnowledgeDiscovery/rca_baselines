...
def get_netwrok_metric(trace_file, pod_name):
    ...
    #
    # try:
    #     pod_reader = pod_reader.reindex(columns=pod_name)
    #     pod_spans = pod_reader.loc[[pod_name], ['SpanID', 'ParentID', 'PodName', 'EndTimeUnixNano']]
    # except:
...
def get_metric_with_time(time, base_dir):
    ...
    for path in path_list:
        if "metric" in path:
            ...
            metric_list = list(metrics.columns)
            metric_list.remove("TimeStamp")
            metric_list.remove("PodName")
            metric_list.remove("Time")
            if 'Date' in metric_list:
                metric_list.remove("Date")
    ...
    pod_num = len(path_list)
    # print(target_list)
    return target_list, pod_num
