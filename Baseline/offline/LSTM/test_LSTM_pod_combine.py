...
if __name__ == '__main__':
    ...
    for metric in metric_names:
        ...
        # cg = generate_causal_graph(X, method='lstm')
        #LSTM based method
        cg = generate_causal_graph(X, method='lstm', args={'hidden': 10, 'context': 10, 'lam': 10., 'lam_ridge': 1e-2, 'lr': 1e-3, 'max_iter': 1000, 'check_every': 50, 'device': torch.device('cuda')})
    ...
    output_file = os.path.join(pathset, 'Pod_level_combine_ranking_combine_{}.csv'.format(dataset))
