...
def gnn(X: np.ndarray, lag: int, layers: list, l1: float, l2: float):
    n_node = X.shape[1]
    cfg = {
        'n_node': n_node,
        'layers': [lag] + layers + [1],
    ...
    model = GNNCI(cfg)
    X = X.transpose()
    W_est = model.train(X, lambda1=l1, lambda2=l2, lag=lag)
...
def generate_causal_graph(X: np.ndarray,
                          ...
                          args: dict = {'lag': 10,
                                        'lambda_w': 1e-3,
                                        'lambda_a': 1e-3})->np.ndarray:
    if method == 'gnn':
        lag = args['lag']
        l1 = args['lambda1']
        l2 = args['lambda2']
        layers = args['layers']
        W_est = gnn(X, lag, layers, l1, l2)
    elif method == 'lstm':
        ...
        W_est = dynotears(X, lag, columns, lambda_w, lambda_a, 0.0)
